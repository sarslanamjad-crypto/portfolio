                    ...
                    <div class="card-body">
                        <form method="post" action="/admin/project-edit/{{$project->id}}" enctype="multipart/form-data">
                            ...
                            <div class="form-floating mb-3">
                                ...
                                <input class="form-control" id="image" type="file" accept=".png, .jpg, .jpeg" name="image" style="padding-bottom:38px">
                                @if ($project->image)
                                    <small class="text-muted d-block mt-1">Current Image: {{ $project->image }}</small>
                                @endif
                                @if ($errors->has('image'))
                                    <span class="text-danger">
                                        {{$errors->first('image')}}
                            ...
                            <div class="form-floating mb-3">
                                <label for="video">Video (Optional, leave blank to keep current, max 50MB)</label>
                                <input class="form-control" id="video" type="file" accept=".mp4, .mov, .ogg, .qt" name="video" style="padding-bottom:38px">
                                @if ($project->video)
                                    <small class="text-muted d-block mt-1">Current Video: {{ $project->video }}</small>
                                @endif
                                @if ($errors->has('video'))
                                    <span class="text-danger">
                                        {{$errors->first('video')}}
